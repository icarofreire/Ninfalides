...
public class BloomFilter<E> implements Serializable {
  ...
  /**
   * Constructs an empty Bloom filter. The optimal number of hash functions (k)
   * is estimated from the total size of the Bloom and the number of expected
   * elements.
   *
   ...
   *
   * @param bitSetSize                     defines how many bits should be used
   *                                       for the filter.
   * @param expectedNumberOfFilterElements defines the maximum number of
   *                                       elements the filter is expected to
   *                                       contain.
   * @param actualNumberOfFilterElements   specifies how many elements have been
   *                                       inserted into the <code>filterData</code>
   *                                       BitSet.
   * @param filterData                     a BitSet representing an existing
   *                                       Bloom filter.
   */
   ...
   * result into 4-byte int's and store them in an array. The digest function is
   * called until the required number of int's are produced. For each call to
   * digest a salt is prepended to the data. The salt is increased by 1 for each
   * call.
   *
   * @param data   specifies input data.
